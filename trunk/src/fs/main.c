...
/* Initialize global variables, tables, etc. */
  ...
  malloc_init(); /* agregado para tener malloc en el fs */
  init_sem_server(); /* inicializamos los semáforos */
}
...
#define MEM_SLOT_SIZE 512
#define MEM_SLOTS 100

typedef struct
{
	char mem_space[MEM_SLOT_SIZE]; /* Slot de memoria reservado */
	int occ; /* Marca ocupado */
} mem_slot_t;

mem_slot_t mem_slots[MEM_SLOTS];

void
malloc_init(void)
{
	/* Inicializo los slots como no usados */
	int i;
...
malloc(size_t size)
{
	...
	if (size > MEM_SLOT_SIZE || size == 0)
		return NULL;
/*	printf("Malloc del newbie!\n");*/
	/* Busco un slot libre */
	for (i = 0; i < MEM_SLOTS; i++)
	{
		if (mem_slots[i].occ == 0)
		{
			slot_used++;
			printf("Malloc: Slots usados %d\n", slot_used);
			mem_slots[i].occ = 1;
			return (void*)mem_slots[i].mem_space;

		}
	}
	
	/* No encontre slots libres */
	return NULL;
}
...
calloc(size_t nmemb, size_t size)
{
	...
	if ((ans = malloc(nmemb * size)) != NULL)
		for (i = 0; i < MEM_SLOT_SIZE; i++)
			((char*)ans)[i] = 0;
...
free(void *ptr)
{
	...
	/* Busco el slot con la dirección que yo le habia pasado */
	for (i = 0; i < MEM_SLOTS; i++)
	{
		if (mem_slots[i].mem_space == (char*)ptr)
		{
			mem_slots[i].occ = 0;
			slot_used--;
			return;
